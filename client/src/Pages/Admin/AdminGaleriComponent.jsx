...
function AdminGaleriComponent() {
    ...
    console.log('Component data: hello', data); 
    ...
    const handleDeleteImage = async () => {
      ...
      try {
        const res = await fetch(
          `/api/image/deleteimage/${imageIdToDelete}/${currentUser._id}`,
          {
            method: 'DELETE',
          }
        );
  ...
  return (
    ...
    {data?.images.map((image, index) => (
        <div key={index} className='border-2 relative group  '>
          {/* Buton başlangıçta opacity-0 ve hover ile opacity-100 olacak */}
          <button 
      ...
      {showModal &&
        ...
        >
          ...
          <div />
          <div 
          className='bg-gray-200 dark:bg-gray-500 p-20 relative rounded-lg  border-4 border-black dark:border-white '>
            <button className='text-red-600 bg-white rounded-full text-3xl absolute right-2 top-2' 
            onClick={() =>
                {setShowModal(false)
              setImageIdToDelete(image._id) }
            }
            >
             <AiFillCloseCircle /></button>
            <div className='text-center'>
              <HiOutlineExclamationCircle 
              className='h-14 w-14 text-yellow-500 dark:text-yellow-300 mb-4 mx-auto' />
              <h3 
              className='mb-5 text-lg  dark:text-gray-100'>
                 Delete Image ?
              </h3>
              <div 
